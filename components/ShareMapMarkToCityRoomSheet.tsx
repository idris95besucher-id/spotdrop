...
import { sendMapMarkToCityRoom } from "@/lib/sendMapMarkMessage";
import {
  fetchRoomCatalog,
...
type ShareMapMarkToCityRoomSheetProps = {
...
export default function ShareMapMarkToCityRoomSheet({
  mark,
  userId,
  isOpen,
  onClose,
  onBack,
  onSent,
}: ShareMapMarkToCityRoomSheetProps) {
  ...
  const sendToRoom = useCallback(
    async (countrySlug: string, citySlug: string) => {
      ...
      const result = await sendMapMarkToCityRoom({
        userId,
        countrySlug,
        citySlug,
        mark,
    ...
    [mark, onClose, onSent, sendingRoomKey, userId]
  );
  ...
  return createPortal(
    <div className={`${bottomSheetLayout.overlay} z-[220]`}>
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="share-map-mark-room-title"
      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-2 border-b border-white/10 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 id="share-map-mark-room-title" className="truncate text-base font-semibold text-white">
              {headerTitle}
            </h2>
