...
export default function PostCommentsSection({
  ...
  }, [comments.length, expanded, loadingComments]);

  useEffect(() => {
    let cancelled = false;

    const loadCurrentUserProfile = async () => {
      if (!expanded || !userId || disabled) {
  ...
  return (
    <section className="space-y-4">
      ...
      {disabled ? (
        <p className="text-xs text-slate-500">Comments are not available on guide preview posts.</p>
      ) : expanded ? (
        <div className="space-y-2">
          {userId ? (
            <div className="flex items-center gap-2 text-xs text-slate-400">
              ...
              <span>Commenting as {publicProfileUsername(currentUserProfile?.username)}</span>
            </div>
          ) : null}
