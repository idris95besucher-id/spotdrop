...
export default function ProfileContentTabs({
  ...
  return (
    <div className="space-y-0">
      <div className="grid grid-cols-3 border-b-2 border-white/10 bg-card">
        <button
          type="button"
      ...
      {activeTab === "collections" ? (
            ...
            return (
              <article key={post.id} className="relative aspect-square overflow-hidden bg-slate-950">
                ...
                {mediaUrl ? (
                  ...
                  >
                    <PostCardMedia post={post} className="h-full w-full" />
                  </PostMediaLink>
                ) : (
                  <PostMediaLink
                    postId={post.id}
                    className="flex h-full w-full items-center justify-center bg-slate-900 px-2 text-center text-xs leading-snug text-slate-300"
                    viewerItems={viewerItems.length > 0 ? viewerItems : undefined}
                    clickedSpot={clickedSpot}
                    onViewerItemDeleted={onPostDeleted}
                  >
        ...
        <div className={`px-4 text-center text-sm text-slate-400 ${compact ? "py-6" : "py-10"}`}>
          {activeTab === "spots" ? (
            <SpotDropSpotsIcon
