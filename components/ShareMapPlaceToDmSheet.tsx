...
import { sendMapPlaceToRecipientsAndGroups } from "@/lib/sendMapPlaceMessage";
import {
  loadSendSpotFriends,
...
type ShareMapPlaceToDmSheetProps = {
...
/**
 * Selection keys are namespaced ("user:<id>" / "group:<id>") because a DM recipient and a
 * group chat live in entirely separate id spaces (profiles vs group_chats) but share one
 * Set-based multi-select — this keeps "Send to N" and toggle logic simple without risking
 * an id collision between the two.
 */
...
export default function ShareMapPlaceToDmSheet({
  place,
  userId,
  isOpen,
  onClose,
  ...
  useEffect(() => {
    ...
    // Same group inbox source as "My chats" (lib/groupChats.ts's get_user_group_inbox RPC) —
    // it's already RLS-scoped to groups the caller belongs to, so no extra filtering is needed.
    void Promise.all([
  ...
  const handleSend = async () => {
    ...
    const result = await sendMapPlaceToRecipientsAndGroups({
      senderId: userId,
      recipientIds,
      groupIds,
      place,
    ...
    if (result.sentCount === 0) {
      setError(result.error ?? t("map.sharePlace.error.sendFailed"));
      return;
    }
  ...
  return createPortal(
    <div className={`${bottomSheetLayout.overlay} z-[220]`}>
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="share-map-place-dm-title"
      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-2 border-b border-white/10 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 id="share-map-place-dm-title" className="truncate text-base font-semibold text-white">
